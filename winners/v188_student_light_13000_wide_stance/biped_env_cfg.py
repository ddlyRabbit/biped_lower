...
#   ACTIONS:      scale=0.5
#   COMMANDS:     lin_vel_x=(-0.6, 0.8) forward, lin_vel_y=(-0.5, 0.5) lateral
...
def feet_air_time_adaptive(
    ...
    threshold_max: float = 0.5,
    height_threshold: float = 0.058,
    switch_step: int = 0,  # immediate impact-based air time
) -> torch.Tensor:
...
BIPED_CFG = ArticulationCfg(
    ...
    actuators={
        "hip_roll": DelayedPDActuatorCfg(
            ...
            effort_limit=50.0, velocity_limit=15.0,
            stiffness=180.0, damping=15.0, armature=0.01,
            friction=0.1,
            min_delay=4, max_delay=15,
        ),
        ...
        "hip_pitch": DelayedPDActuatorCfg(
            ...
            stiffness=180.0, damping=20.0, armature=0.025,
            friction=0.1,
            min_delay=4, max_delay=15,
        ),
        "knee": DelayedPDActuatorCfg(
            ...
            effort_limit=100.0, velocity_limit=15.0,
            stiffness=180.0, damping=10.0, armature=0.3,
            friction=0.1,
            min_delay=4, max_delay=15,
        ),
        "foot_pitch": DelayedPDActuatorCfg(
            joint_names_expr=[".*foot_pitch.*"],
            effort_limit=30.0, velocity_limit=15.0,
            stiffness=30.0, damping=2.8, armature=0.1,
            friction=0.1,
            min_delay=4, max_delay=15,
        ),
...
class CommandsCfg:
    base_velocity = base_mdp.UniformVelocityCommandCfg(
        ...
        ranges=base_mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.6, 0.8),      # forward=+X (biased positive)
...
class CurriculumsCfg:
    push_force_levels = CurrTerm(
        ...
        params={
            ...
            "max_velocity": [0.7, 0.7],
            "interval": 200 * 24,
            "starting_step": 200 * 24,      # start after 200 iterations
        },
    )
